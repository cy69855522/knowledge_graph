...
> 💡 这里用 PyTorch 的 **行向量约定** `Y = X @ W`（每行是一个样本，方便处理 Batch）。
> 教科书常写 `y = Wx`（列向量），本质等价，只是所有矩阵转置了。

**前向：** $Y_{ij} = \sum_{k} X_{ik} \cdot W_{kj}$

其中下标含义：
- **$i$**: 第几个样本（$1 \leq i \leq B$，Batch 维度）
- **$k$**: 输入的第几个特征（$1 \leq k \leq D_{in}$，求和维度）
- **$j$**: 输出的第几个特征（$1 \leq j \leq D_{out}$，输出维度）

**已知：** 损失对输出的梯度 $\frac{\partial L}{\partial Y}$（由后面的层传回来的）

**① 求 $\frac{\partial L}{\partial X}$（把梯度继续往前传 🔙）：**

用链式法则：

$$\frac{\partial L}{\partial X_{ik}} = \sum_{j} \frac{\partial L}{\partial Y_{ij}} \cdot \frac{\partial Y_{ij}}{\partial X_{ik}}$$

从前向公式知道 $\frac{\partial Y_{ij}}{\partial X_{ik}} = W_{kj}$，代入：

$$\frac{\partial L}{\partial X_{ik}} = \sum_{j} \frac{\partial L}{\partial Y_{ij}} \cdot W_{kj} = \sum_{j} \left(\frac{\partial L}{\partial Y}\right)_{ij} \cdot (W^T)_{jk}$$

认出来了！这就是矩阵乘 👉 **$\frac{\partial L}{\partial X} = \frac{\partial L}{\partial Y} \cdot W^T$**

```
dX    =  dL/dY  @  W^T
[B,D_in] = [B,D_out] @ [D_out,D_in]   ✅ Shape 对得上！
```

**② 求 $\frac{\partial L}{\partial W}$（用来更新权重 ⚖️）：** 同理可推：

$$\frac{\partial L}{\partial W_{kj}} = \sum_{i} X_{ik} \cdot \frac{\partial L}{\partial Y_{ij}} \quad \Rightarrow \quad \frac{\partial L}{\partial W} = X^T \cdot \frac{\partial L}{\partial Y}$$

```
dW      =  X^T    @  dL/dY
[D_in,D_out] = [D_in,B] @ [B,D_out]   ✅ Shape 对得上！
```

**📸 直觉理解：** W 在前向时把 X "投影" 到 Y 空间。反向时，梯度要沿同一条路**投影回去**，所以乘 $W^T$。就像光照到屏幕 (前向 `X@W`)，反射回来 (反向 `dY@W^T`)，镜子就是 $W^T$ 🪞

#### 🧩 记不住公式？用 Shape 拼图法现场推！

> 💡 **不用背公式**，只需要记住 `Y = X @ W`，然后用 Shape 凑就行。
> 核心规则：**梯度的 Shape 必须和原变量的 Shape 一样**（不然没法更新！）
...
W:     [D_in, D_out]
Y:     [B, D_out]
dL/dY: [B, D_out]      ← 和 Y 同 Shape，由后面的层传来
```

**① 求 dX，Shape 必须是 `[B, D_in]`：**
手头有 `dL/dY [B, D_out]` 和 `W [D_in, D_out]`，怎么凑？
```
[B, D_in] = [B, D_out] @ [D_out, D_in]
              dL/dY     @    W^T          ✅ W 转置一下就对了！
```

**② 求 dW，Shape 必须是 `[D_in, D_out]`：**
手头有 `X [B, D_in]` 和 `dL/dY [B, D_out]`，怎么凑？
```
[D_in, D_out] = [D_in, B] @ [B, D_out]
                   X^T     @   dL/dY     ✅ X 转置一下就对了！
```

> 🎯 **面试技巧：** 在纸上画出 Shape，像拼拼图一样把中间维度对齐，5 秒钟就能推出来，比死记硬背靠谱 💪
